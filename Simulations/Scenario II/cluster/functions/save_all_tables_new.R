...
save_all_tables<-function(muhaz.hr.df.list,true.hr,path.results,n,tau,BSE,perc.ci2,BSE.cox,perc.ci.cox){
  #print(muhaz.hr.df.list)
  ...
  #BSE<-t(BSE)
  names(BSE)<-paste("BSE_sim_",c(1:length(muhaz.hr.df.list)),sep = "")
  kernel.results<-cbind(kernel.results,BSE)
  #print(BSE)
  #The true HR
  ...
  for( i in 1:length(perc.ci2)){
    ...
    for( j in 1:nrow(perc.ci2[[i]])){
      if((!is.na(perc.ci2[[i]][j,1])&&(perc.ci2[[i]]$true.hr[j]>=perc.ci2[[i]][j,1]))&&(!is.na(perc.ci2[[i]][j,1])&&(perc.ci2[[i]]$true.hr[j]<=perc.ci2[[i]][j,2]))){
