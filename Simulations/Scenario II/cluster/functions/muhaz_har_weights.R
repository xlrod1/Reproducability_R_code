...
muhaz_hr_weights<-function(data,theta, n.est.grid = 51,b.cor="n",end.time=NA,min.time =NA,kern="e"){
  ...
  df<-data.frame(time=data.hazard$time,HR=causal.hazard(data.hazard$haz.1,data.hazard$haz.0,
                                                        ...
                                                        theta=theta),method="muhaz-weighted")
  
 df.full<-data.frame(time=data.hazard$time,HR=causal.hazard(data.hazard$haz.1,data.hazard$haz.0,
                                                            data.hazard$cumsum1.ver3,data.hazard$cumsum0.ver3,
                                                            theta=theta),haz.1=data.hazard$haz.1,haz.0=data.hazard$haz.0, 
                                                            cumsum1=data.hazard$cumsum1.ver3, cumsum0=data.hazard$cumsum0.ver3,method="muhaz-weighted")
