condition.on.x.saving.boot<-function(n=5000,
                                    tau=0.7,
                                     beta.x=log(0.5),
                                     lamdas=1,
                                     beta.a=log(0.5),
  ...
  for( i in 1:n.simulate){
    ...
    if (parallel=="no"){
     ...
     errors.file[i,1:n.est.grid]<-data.frame(results$t )%>%
      ...
      BSE[[i]]<-apply(results$t, 2, function(u){sqrt(var(u,na.rm = TRUE))})
      perc.ci2[[i]]<-data.frame(t(apply(results$t, 2, function(u){quantile(u, probs = c(.025, .975,0.25,0.75), type = 6,na.rm = TRUE)})))
      ###################################
      #Bootstrap for cox
      ...
      BSE.cox[[i]]<-apply(results$t, 2, function(u){sqrt(var(u,na.rm = TRUE))})
      #names(BSE.cox[i])<-paste("sim_",i,sep = "")
      perc.ci.cox[[i]]<-data.frame(t(apply(results$t, 2, function(u){quantile(u, probs = c(.025, .975,0.25,0.75), type = 6,na.rm = TRUE)})))
      #print(BSE.cox[i])
    }
    #parallel computation for cluster!
      ...
      results$t<-apply(results$t,c(1,2),function(x){ifelse(x>10||x<(-10),NA,x)})
      BSE.cox[[i]]<-apply(results$t, 2, function(u){sqrt(var(u,na.rm = TRUE))})
      perc.ci.cox[[i]]<-data.frame(t(apply(results$t, 2, function(u){quantile(u, probs = c(.025, .975,.25,.75), type = 6,na.rm = TRUE)})))
